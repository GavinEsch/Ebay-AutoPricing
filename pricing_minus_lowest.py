...
for _, row in df.iterrows():
    ...
    # --------------------------------------------
    first_ebay_price   = row[ebay_price_start_place]
    last_ebay_price    = row[len(row) - 1]

    # ----------------------------------------------------------
    # ❷ All stats are now based on the *remaining* eBay columns
    # ----------------------------------------------------------
    ebay_prices = (
        pd.to_numeric(row[ebay_price_start_place + 1 :],  # skip the first one
                      errors='coerce')
          .dropna()
          .tolist()
    )
...
results_df.to_csv('pricing_analysis_minus_lowest.csv', index=False)
